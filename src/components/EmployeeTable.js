...
function EmployeeTable() {
  ...
  useEffect(() => {
    ...
    function fetchEmployeeTable() {
      ...
      axios.get(process.env.REACT_APP_REQUEST_URL + `ViewUsersByStoreID?storeID=${curStoreID}`, {
        headers: {
          [process.env.REACT_APP_HEADER]: process.env.REACT_APP_API_KEY
        }
      })
            ...
            {employees
              ...
              .map(employee => (
                <tr
                  ...
                  className={`cursor-pointer ${selectedRow === employee.ID ? 'bg-gray-100' : (employee.enabled ? 'hover:bg-gray-100' : 'bg-gray-300')
                    }`}
